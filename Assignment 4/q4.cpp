...
void enqueue(char queue[], int &front, int &rear, int max_size, char item) {
    if ((rear + 1) % max_size == front) return;  // queue full
    if (front == -1) front = 0;
...
char dequeue(char queue[], int &front, int &rear, int max_size) {
    if (front == -1) return '\0';  // queue empty
...
void findFirstNonRepeating() {
    ...
    for (int i = 0; i < (int)str.length(); i++) {
        ...
        // Remove all repeating chars from front of queue
        while (front != -1 && charCount[getFront(q, front) - 'a'] > 1) {
            dequeue(q, front, rear, max_size);
        }

        cout << "First non-repeating character for prefix \"" 
             << str.substr(0, i + 1) << "\" is: ";
